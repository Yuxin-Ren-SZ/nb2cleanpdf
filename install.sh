...
#
# Environment: NB2CLEANPDF_URL overrides where the script is downloaded from
# when install.sh runs outside a clone (default: the main branch on GitHub).
# ---------------------------------------------------------------------------
set -eu

NAME=nb2cleanpdf
URL=${NB2CLEANPDF_URL:-https://raw.githubusercontent.com/Yuxin-Ren-SZ/nb2cleanpdf/main/nb2cleanpdf}
...
usage() {
  if [ -f "$0" ]; then sed -n '3,19p' "$0" | cut -c3-
  else info "options: --bin-dir DIR | --prefix DIR | --link | --force | --uninstall"; fi
}

BIN_DIR=$HOME/.local/bin LINK=0 FORCE=0 UNINSTALL=0
while [ $# -gt 0 ]; do
  case $1 in
...
if [ -z "$SRC" ]; then
  ...
  curl -fsSL "$URL" -o "$TMPF" || die "download failed: $URL"
  SRC=$TMPF
fi
